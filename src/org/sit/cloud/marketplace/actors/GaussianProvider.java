		...
		for(String vmId : runningVmIdToVmMap.keySet()){
			if(isBadProvider){
				if(TimeKeeper.getTime() >= TimeKeeper.VIOLATION_START)// && TimeKeeper.getTime() <= TimeKeeper.VIOLATION_END)
					map.put(vmId, new QoS(gaussianDistribution.nextGaussianValueForQoSDegradation()*getPromisedAvailability(), (2-gaussianDistribution.nextGaussianValueForQoSDegradation())*getPromisedBandwidth()));
				else
